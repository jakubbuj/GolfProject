...
public class GolfAI {
    ...
    private float[][] invertMatrix(float[][] matrix) {
        ...
        float d = matrix[1][1];
        float det = a * d - b * c;
        if (det == 0) return null; // Check for non-invertible matrix
        return new float[][]{
            {d / det, -b / det},
            {-c / det, a / det}
        };
    ...
    public void update() {
        ...
        AIball.getPosition().y = (float) (physicsEngine.terrainHeight + 0.2f); // Keeping the ball slightly above the terrain
    }
}
