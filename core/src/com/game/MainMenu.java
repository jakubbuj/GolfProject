...
public class MainMenu implements Screen {
    ...
    public void render(float delta) {
         ...
         if (Gdx.input.getX() < x + EXIT_BUTTON_WIDTH && Gdx.input.getX() > x && GolfGame.HEIGHT - Gdx.input.getY() < EXIT_BUTTON_Y + EXIT_BUTTON_HEIGHT && GolfGame.HEIGHT - Gdx.input.getY() > EXIT_BUTTON_Y) {
             game.batch.draw(exitButtonActive, GolfGame.WIDTH / 2 - EXIT_BUTTON_WIDTH / 2, EXIT_BUTTON_Y, EXIT_BUTTON_WIDTH, EXIT_BUTTON_HEIGHT);
             if (Gdx.input.isTouched()) {
                 Gdx.app.exit();
             }
         } else {
         game.batch.draw(exitButtonInactive, GolfGame.WIDTH / 2 - EXIT_BUTTON_WIDTH / 2, EXIT_BUTTON_Y, EXIT_BUTTON_WIDTH, EXIT_BUTTON_HEIGHT);
         }
 
         //handling play button - sets the screen to OptionsScreen
         x = GolfGame.WIDTH / 2 - PLAY_BUTTON_WIDTH / 2;
         if (Gdx.input.getX() < x + PLAY_BUTTON_WIDTH && Gdx.input.getX() > x && GolfGame.HEIGHT - Gdx.input.getY() < PLAY_BUTTON_Y + PLAY_BUTTON_HEIGHT && GolfGame.HEIGHT - Gdx.input.getY() > PLAY_BUTTON_Y) {
             game.batch.draw(playButtonActive, GolfGame.WIDTH / 2 - PLAY_BUTTON_WIDTH / 2, PLAY_BUTTON_Y, PLAY_BUTTON_WIDTH, PLAY_BUTTON_HEIGHT);
             if (Gdx.input.isTouched()) {
                 game.setScreen(new OptionsScreen(game));
             }
         } else {
         game.batch.draw(playButtonInactive, GolfGame.WIDTH / 2 - PLAY_BUTTON_WIDTH / 2, PLAY_BUTTON_Y, PLAY_BUTTON_WIDTH, PLAY_BUTTON_HEIGHT);
         }

         //handling settings button - sets the screen to SettingsScreen
         x = GolfGame.WIDTH / 2 - SETTINGS_BUTTON_WIDTH / 2;
         if (Gdx.input.getX() < x + SETTINGS_BUTTON_WIDTH && Gdx.input.getX() > x && GolfGame.HEIGHT - Gdx.input.getY() < SETTINGS_BUTTON_Y + SETTINGS_BUTTON_HEIGHT && GolfGame.HEIGHT - Gdx.input.getY() > SETTINGS_BUTTON_Y) {
             game.batch.draw(settingsButtonActive, GolfGame.WIDTH / 2 - SETTINGS_BUTTON_WIDTH / 2, SETTINGS_BUTTON_Y, SETTINGS_BUTTON_WIDTH, SETTINGS_BUTTON_HEIGHT);
             if (Gdx.input.isTouched()) {
                 game.setScreen(new SettingsScreen(game));
             }
         } else {
         game.batch.draw(settingsButtonInactive, GolfGame.WIDTH / 2 - SETTINGS_BUTTON_WIDTH / 2, SETTINGS_BUTTON_Y, SETTINGS_BUTTON_WIDTH, SETTINGS_BUTTON_HEIGHT);
         }
